...
#S: above truc's version changed as:
    ...
    def visit_FuncCall(self, n):
        ...
        if fref == core.common.changeID['pthread_exit']:
            ...
            return 'goto __exit_%s ' % (self.currentFunction[1]) #S: old (self.currentFunction[-1]), thread name is the name at the bottom of currentFunction stack
        ...
        if self.____needsInlining(fref):
            ...
            if self.Parser.funcIsVoid[fref]: 
                ...
                #    #self.funcInlinedCount[fref] =0  #S: replaced by decreasing counter on returning 
                    s = '__cs_retval_%s_%s' % (fref,self.indexStack[index])   #S: this is needed if function is not void, and is a fake assignment, will never occur and choice of var to assign is safe in the sense that it is of the same type of the return value and is certainly declared at this point.
    ...
    def visit_Return(self, n):
        if len(self.indexStack) > 0:
            if self.Parser.funcIsVoid[self.currentFunction[-1]]:
                ...
                return '__cs_retval_%s_%s = %s; goto __exit_%s_%s;' % (self.functionStack[-1],self.indexStack[-1], self.visit(n.expr), self.functionStack[-1], self.indexStack[-1]) # non-void
                ...
                if initType == 1:
                    ...
                    if processInit:
                        ...
                        if self.keepstaticarray:
                            ...
                            #S: init local vars 
                            if self.init == 1:
                                  s += '__cs_init_scalar(& %s, (sizeof(%s)*%s));' % (name, vartype, self._totalSize(self.currentFunction[-1], n.name))  #S: n.name --> name
                            elif self.init == 0:     
                                  s += n.name + ' = (%s %s) %s(sizeof(%s)*%s)' % (vartype, stars, core.common.changeID['malloc'], vartype, self._totalSize(self.currentFunction[-1], name))  #S: original transf.  #S: n.name --> name
...
#truc's version                label = label +'_' + self.currentFunction[-1] + '_' + str(count)
            ...
            if self.inlinedStack[-1] == '':  # If this statement doesn't contain inlined function
                ...
                #original = original.replace('(DELETETHIS;\n)', '')
                original = original.replace('(DELETETHIS)', '0')  #S: added to handle cases when the function call is part of an expression
                #original = re.sub('(DELETETHIS;\n)|(DELETETHIS)', '', original)
        ...
        if fcall_ast_node.args is not None:
            ...
            for expr in fcall_ast_node.args.exprs:   # for each parameter in the function call
                ...
                if (isinstance(expr, pycparser.c_ast.UnaryOp) and
                    ...
                    pname = self.updateName(self.Parser.funcParams[fref][paramNo]) #S: added to handle var renaming in each inlining of functions
                #pname=''
                ...
                if self.Parser.varKind[fref,p] == 'p':
                    ...
                    if not self.__isPointerToFunction(fref,p) and not self.__isArray(fref,p):
                        ...
                        if 'const' in self.Parser.varTypeUnExpanded[fref,p]:   
                              fInput += 'static %s %s = %s; ' % (self.Parser.varTypeUnExpanded[fref,p], pname, self.currentFunctionParams[-1][i])
                        else:
                              fInput += 'static %s %s; %s = %s; ' % (self.Parser.varTypeUnExpanded[fref,p], pname, pname, self.currentFunctionParams[-1][i])
                        i += 1
...
#                            stars += '*'
                        ...
                        x = self.Parser.varTypeUnExpanded[fref,p].replace('(*)', '(*%s)' % p) #S: added non in pre-debian
...
#                        fInput += 'static %s %s%s; %s = %s; ' % (self.Parser.varTypeUnExpanded[fref,p], stars, p, p, self.currentFunctionParams[-1][i])
        ...
        #S
                        ...
                        self.currFuncPtrParamMap[pname]=self.removeCasting(self.currentFunctionParams[-1][i]) #S: replace above lines, parameter disappears from code and just save value for inlining at the function call  
    ...
    def ____needsInlining(self, f):
        ...
        if self.currFuncPtrParamMap and f in self.currFuncPtrParamMap.values():
           b = True
	#if f == "__cs_safe_malloc": print self.currFuncPtrParamMap  #QUI
        return (b or   #S: added disjunct to handle function pointers
              (f in self.Parser.funcBlock and    # defined functions need to be inlined when called (if at all)
...
#
#        # Final check
#        print "%s --> is called by %s" %(fname,self.Parser.callReferences[fname])
