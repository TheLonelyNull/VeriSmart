  ...
  if instanceIterator == None: 
    ...
    if env['execution-mode'] == 'sequential':
        ...
        while initalTimeout <= env['timelimit']:
            ...
            pool_size = env['cores']
            print 'Analyzing %s instance(s) with %ss timeout for %s processes' % (
    ...
    if env['execution-mode'] == 'sequential':
        ...
        instanceLog = ''
        instanceTimeBackend = 0
        print 'Analyzing instance(s) SEQUENTIALLY with %ss timeout for each instance' % (
              initalTimeout)
        print '============================================================================'
        sys.stdout.flush()
            # Run backend through seq file list
        ...
        pool_size = multiprocessing.cpu_count()  # for cores of cpu
        pool_size = env['cores']
        print 'Analyzing instance(s) with %ss timeout for %s processes' % (
              initalTimeout, pool_size)
        print '============================================================================'
        sys.stdout.flush()
        pool = multiprocessing.Pool(processes=pool_size, initializer=start_process)
        # create jobs for all workers
        ...
        try:
             ...
             for job in jobIterator:
                #while results.empty(): print("PIPPO")
          #      if results.size() == 0: 
          #           event.wait()
                out = results.get()
          #      event.clear()
